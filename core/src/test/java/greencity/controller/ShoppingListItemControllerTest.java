...
public class ShoppingListItemControllerTest {
    ...
    void findAllByUserTest() throws Exception {
        long id = 1L;
        mockMvc.perform(get(STR."\{shoppingListItemLink}/\{id}/get-all-inprogress")
                .param("lang", "en"))
                .andExpect(status().isOk());

        when(shoppingListItemService.findInProgressByUserIdAndLanguageCode(id, "en"))
                .thenReturn(new ArrayList<>());
        verify(shoppingListItemService).findInProgressByUserIdAndLanguageCode(id, "en");
        assertTrue(shoppingListItemController.findInProgressByUserId(id, "en").getBody().isEmpty());
    }

    @Test
    void getUserShoppingListItemWithoutLanguageParamTest() throws Exception {
        long habitId = 1L;
        mockMvc.perform(get(shoppingListItemLink + "/habits/" + habitId + "/shopping-list"))
                .andExpect(status().isOk());

        verify(shoppingListItemService).getUserShoppingList(null, habitId, "en");
    }

    @Test
    void getUserShoppingListItemsWithLanguageParamTest() throws Exception {
        long habitId = 1L;
        mockMvc.perform(get(shoppingListItemLink + "/habits/" + habitId + "/shopping-list?lang = en"))
                .andExpect(status().isOk());
    ...
    void saveUserShoppingListItemWithoutLanguageParamTest() throws Exception {
        ...
        mockMvc.perform(post(shoppingListItemLink + "?habitId=1", 1)
